...
- The project-scoped server is registered in the local tool config under the name `databricks`.
...
- `.env` must contain both `DATABRICKS_ACCESS_TOKEN` and `DATABRICKS_HOST`. Never copy the token into tool configs, scripts, logs, or documentation. `.gitignore` excludes `.env` and the downloaded MCP checkout.
...
- Confirm the `databricks` server is enabled after any MCP configuration change, then restart the session.
