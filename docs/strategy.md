...
| Model Serving | **Frontier models are rate-limited to 0.** Claude Opus 4.8, Claude Sonnet 5, Gemini 3.5 Flash all return 403 despite listing as READY. Open-weight models (Llama 3.3 70B, Qwen3-Next 80B, Gemma 3) work at sub-second latency. All 3 embedding models work | Rung 3 uses open-weight in-workspace models. Verified by direct call, not inference. (The shipped referee runs in rules mode; no external model was needed) |
